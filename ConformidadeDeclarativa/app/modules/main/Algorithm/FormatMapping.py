...
def format_inconformances(process_conformance, access_conformance, resource_conformance, activity_conformance):
    '''
    Formats the non-conformances found in each check to print. Used for backend testing.
    '''
...
def non_conformance_patterns_mapping(process_violations, access_violations, resource_violations, unexpected_activities, activities_stats, log_size, duration, process_log):
    ...
    for act in resource_violations['IllegalTeamActivity']:
        patterns['Illegal activity'].append({'activity': act[0], 'case_id': act[1], 'resource': act[2], 'instance': str(act[3])})
